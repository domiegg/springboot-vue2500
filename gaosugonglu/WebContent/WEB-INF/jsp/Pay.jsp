	...
	<div class="formbody">
		...
		<div id="usual1" class="usual">
			...
			<div id="tab1" class="tabson">
				...
				<form action="pay/informationUpdate" method="POST"
					onsubmit="return checkSubmit();">
					<input type="hidden" name="id" value="${pay['id'] }">
					<ul class="forminfo">
						<li><label>收费名称<b>*</b></label><input id="name" value="${pay['name'] }"
							name="name" type="text" class="dfinput" placeholder="请填写收费名称"
							style="width: 518px;" /></li>
						<li><label>车辆号牌<b>*</b></label><input id="code" value="${pay['code'] }"
						...
						<li><label>车辆类型<b>*</b></label><div class="vocation"> <c:if test="${message!=null }">
								...
								<c:forEach items="${ctypeList }" var="ctype">
									<c:if test="${ctype['id'] eq pay['ctype']['id']}" var="s1">
						...
						<li><label>收费类型<b>*</b></label><div class="vocation"> <c:if test="${message!=null }">
								<c:out value="${message }"></c:out>
							</c:if> <select class="select2" name="ptype_id" id="ptype_id">
								<c:forEach items="${ptypeList }" var="ptype">
									<c:if test="${ptype['id'] eq pay['ptype']['id']}" var="s2">
										<option value="${ptype['id'] }" selected="selected">${ptype['name'] }</option>
									</c:if>
									<c:if test="${!s2 }">
						...
						<li><label>收费站<b>*</b></label><div class="vocation"> <c:if test="${message!=null }">
								...
								<c:forEach items="${stationList }" var="station">
									<c:if test="${station['id'] eq pay['station']['id']}" var="s3">
						...
						<li><label>收费金额<b>*</b></label><input id="price" value="${pay['price'] }"
							name="price" type="text" class="dfinput" placeholder="请填写收费金额"
							style="width: 518px;" /></li>
						<li><label>备注<b>*</b></label><textarea id="content7" name="content">${pay['content'] }</textarea></li>

...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/stationAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("station/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="station/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${stationList.datas }" var="station" varStatus="vg">
						<tr>
							<td><c:out value="${station['name']}"></c:out></td>
							<td><c:out value="${station['createtime']}"></c:out></td>
							<td><a href="javascript:;" onclick="update(${station['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${station['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="station/informationLoad?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="station/informationLoad?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="station/informationLoad?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="station/informationLoad?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="station/informationLoad?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="station/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {

	...
	<form action="user/query" method="POST"
		...
		<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>用户名</label><input type="text" class="scinput1"
						name="loginName" value="${user['loginName'] }"></li>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="user/query?pageNow=${pager.pageNow }&loginName=${user['loginName']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="user/query?pageNow=${pager.pageNow-1 }&loginName=${user['loginName']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="user/query?pageNow=${vs.count }&loginName=${user['loginName']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="user/query?pageNow=${count }&loginName=${user['loginName']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="user/query?pageNow=${count }&loginName=${user['loginName']}">${count}</a></li>
